...
군집 1: 에너지 및 자원 관련 키워드 → 에너지(energy), 재료(materials), 물(water), 가스(gas), 기술(technologies)
군집 2: 역사 및 지리 관련 키워드 → 역사(history), 역사가(historian), 고대(ancient), 중세(medieval), 지리(geography)
군집 3: 생물학 및 환경 과학 관련 키워드 → 동물(animals), 종(species), 환경(environmental), 과학(science), 생물학(biology)
'''
import json
import re
import numpy as np
